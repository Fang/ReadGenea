...
\code{[} allows matrix style manipulations. If the first column (the timestamp column) is included, a data.frame is produced with the timestamp as a "RGtime" object via \code{\LinkA{convert.time}{convert.time}}. This allows improved plotting of time axes. If j is not specified, an "AccData" object is returned.
