...
A time representation of when the processing is to occur, see \code{\LinkA{reformat.time}{reformat.time}} for possible representations, or 
...
A time representation of when to end processing, see \code{\LinkA{reformat.time}{reformat.time}} for possible representations, or 
...
\item[\code{tformat}] 
A character string indicating which (numerical) format the timestamps should be saved in.  See \code{\LinkA{reformat.time}{reformat.time}} for more details.
...
\code{correct.z}: a boolean variable indicating whether to systematic correction of the z-axis accelerometer values.  See Details for more information.  Defaults to FALSE.\\{}

\code{gainfactor}: an initial value for the multiplication factor for z-axis gain correction (used if \code{correct.z==TRUE}).  Defaults to 1.1.\\{}
...
\code{nchunks}: the number of chunks in which to do processing of the data pages.  If not specified, this is calculated given a \code{chunksize} of 500 pages.\\{}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{convert.hexstream}{convert.hexstream}}, \code{\LinkA{reformat.time}{reformat.time}} 
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
