...
as.RGtime <- function(x, format = NULL, ...){
if (is.character(x)){
...
function(x,..., na.rm)
{
	...
	att = attributes(x)
	if (!ok)
		stop(.Generic, " not defined for RGtime objects")
...
{
    if (nargs() == 1) 
        stop("unary ", .Generic, " not defined for RGtime objects")
...
if (inherits(e1, "RGtime")){
	att = attributes(e1)
	if ((!unitpres) && inherits(e2, "RGtime")) att = NULL
} else {
	att = attributes(e2)
}
if (!arith) stop(.Generic, " not defined for RGtime objects")
...
#ZF#    if(all(is.na(x)) || any(x[!is.na(x)] >= 1))
 #ZF#       cat("Time in days:\n")
    x <- format.RGtime(x, format = format)
...
Axis.RGtime <- function(x = NULL, at = NULL, ..., side, labels = TRUE){
axis.RGtime(side = side, x = x, at = at, labels = labels, ...)
}
...
{
    if (is.null(at)){
#	    if(!inherits(x, "RGtime")) x <- convert.time(x)
	    att <- NULL
	    if (inherits(x, "RGtime")) att <- attributes(x)
	    ...
	    attributes(tmp) = att
	    at = pretty.RGtime(tmp, n)
	} else {
	    if(!inherits(at, "RGtime")) at <- convert.time(at)
	}
