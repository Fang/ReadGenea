...
if (!bw){
plot(convert.time(tmp2[ind,1]) -> x, -acos(tmp2[ind,3] / sqrt(rowSums(tmp2[ind,-1]^2)) ) *180/pi +90 ->y, col =col[ floor( length(col)* (sign(-tmp2[ind,2]) * 180 *acos(-tmp2[ind,4] / sqrt(rowSums(tmp2[ind,-c(1,3)]^2)))/pi +180)/360   ) + 1 ], ylim = c(-90, 100), xlab = "Time", ylab="Up/Down", pch=".", cex= 2, yaxt = "n"); abline(h = c(-2:2) * 45, lty= 2); axis(2, at = 45 * -2:2)
...
plot(convert.time(tmp2[ind,1]) -> x, -acos(tmp2[ind,3] / sqrt(rowSums(tmp2[ind,-1]^2)) ) *180/pi +90 ->y, ylim = c(-90, 100), xlab = "Time", ylab="Up/Down", pch=".", cex= 2, yaxt = "n"); abline(h = c(-2:2) * 45, lty= 2); axis(2, at = 45 * -2:2)
...
beg =  bapply.basic(convert.time(tmp2[ind, 1]), floor(length(degs)/100), min)
end =  bapply.basic(convert.time(tmp2[ind, 1])[-1], floor(length(degs)/100), max)
...
tmp = convert.time(seq(tmp2[1,1] , quantile(tmp2[,1], 0.2), len = 361) )
